...
import { dictTypeApi } from '@/service/api';
...
type Model = Pick<Api.System.DictType, 'name' | 'code' | 'status' | 'remark'>;
...
async function handleSubmit() {
  ...
  if (props.operateType === 'add') {
    const { error } = await dictTypeApi.add(model.value);

    if (!error) {
      window.$message?.success($t('common.addSuccess'));
    }
  } else if (props.rowData) {
    const { error } = await dictTypeApi.update(props.rowData.id, model.value);
    ...
    <NDrawerContent :title="title" :native-scrollbar="false" closable>
      <NForm ref="formRef" :model="model" :rules="rules">
        <NFormItem :label="$t('page.system.dict.form.name')" path="name">
          <NInput v-model:value="model.name" :placeholder="$t('page.system.dict.form.name')" />
        </NFormItem>
        <NFormItem :label="$t('page.system.dict.form.code')" path="code">
          <NInput v-model:value="model.code" :placeholder="$t('page.system.dict.form.code')" />
        </NFormItem>
        ...
        <NFormItem :label="$t('page.system.dict.form.remark')" path="remark">
          <NInput v-model:value="model.remark" type="textarea" :placeholder="$t('page.system.dict.form.remark')" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace :size="16">
